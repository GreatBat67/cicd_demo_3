# Generates DCM manifest.yml inside dcm_automation/ (where sources/definitions/ lives)
...
def get_project_root():
    ...
    for directory in [start] + list(start.parents):
        candidate = directory / "dcm_automation"
        if candidate.exists() and (candidate / "sources").exists():
            return candidate

    raise RuntimeError(
        "Could not locate dcm_automation/ directory with sources/. "
...
def main():
    ...
    # Write manifest.yml inside dcm_automation/ (same level as sources/)
